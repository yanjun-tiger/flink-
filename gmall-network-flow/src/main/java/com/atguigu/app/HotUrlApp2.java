...
//        apachLogDS.print("apachLogDS");
    ...
    private static class UrlCountProcessFunc extends KeyedProcessFunction<Long,UrlViewCount,String>{
        ...
        //定义集合状态用于存放同一个窗口中的数据 实现去重的功能
        //<UK, UV> the keys in the state; the values in the state.
        private MapState<String,UrlViewCount> mapState; //保存UrlViewCount，是因为可以从这个对象中取出它的属性
        ...
        public void processElement(UrlViewCount value, Context ctx, Collector<String> out) throws Exception {
            ...
            //使用上下文来注册定时器,用于处理状态中的数据
            ctx.timerService().registerEventTimeTimer(value.getWindowEnd()+1L);
            //使用上下文来注册定时器，用来处理状态中的数据的清除
        ...
        public void onTimer(long timestamp, OnTimerContext ctx, Collector<String> out) throws Exception {
            ...
            //1 获取状态中的数据
            Iterator<Map.Entry<String, UrlViewCount>> iterator = mapState.entries().iterator(); //这样可以获取所有的key-value对
